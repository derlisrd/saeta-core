...
<main class="flex items-center justify-center bg-gray-100 dark:bg-dark">
    <div class="max-w-md w-full p-8 bg-white dark:bg-dark rounded-xl shadow-lg mt-12">
        <h1 class="text-center text-xl font-extrabold text-gray-900 dark:text-white uppercase">
            Registrarse
        </h1>
        <form class="mt-8 space-y-6" action="{{ route('signup_submit') }}" method="POST">
            @csrf
           <div class='flex flex-col gap-3'>
            ...
            <button type='submit' class='bg-green-800 hover:bg-green-600 transition-colors ease-in-out duration-200 text-white font-bold py-2 px-4 rounded-lg w-full'>Registrarse</button>
        </div>
        </form>
    </div>
</main>
